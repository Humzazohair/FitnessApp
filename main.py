...
def save_artifacts(model, model_path, model_config, class_mapping):
    ...
    torch.save({
        ...
        **model_config,
        'class_to_exercise': class_mapping,
        'exercise_to_class': {v: k for k, v in class_mapping.items()}
    }, model_path)
    
    with open(model_path.parent / 'exercise_classes.json', 'w') as f:
...
def main():
    ...
    sequences, labels, class_to_exercise = loader.load()
    print(f"\nTotal sequences: {len(sequences)}")
    print(f"Classes: {list(class_to_exercise.values())}")
    ...
    # Create model
    num_classes = len(class_to_exercise)
    model = create_model(
    ...
    print(f"Training {config.MODEL_TYPE.upper()} with {sum(p.numel() for p in model.parameters()):,} parameters\n")
    ...
    model_path = config.MODEL_DIR / f'exercise_classifier_{config.MODEL_TYPE}.pth'
    save_artifacts(trainer.model, model_path, model_config, class_to_exercise)
